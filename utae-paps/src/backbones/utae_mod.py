...
class UTAE(nn.Module):
    def __init__(
        ...
        # # make last layer tanh not ReLU
        # self.out_conv.conv.conv[-1] = torch.nn.Tanh()
        self.final_downsample = torch.nn.AvgPool2d(kernel_size=2)
    ...
    def forward(self, input, batch_positions=None, return_att=False):
        ...
        # shape is [B x T x encoder_widths[0] x H x W]
        feature_maps = [out]
        # SPATIAL ENCODER
        for i in range(self.n_stages - 1):
            # shape is [B x T x encoder_widths[1 + i] x H/(2^(i+1) x W/(2^(i+1)]
            # cuts H/W in half and makes feature maps that follow encoder_widths
        ...
        for i in range(self.forecast_steps):
            # TEMPORAL ENCODER
            # out is same shape as feauture_maps[-1] but it squashes temporal dim
            # att is of shape [n_head, B, T, H, W], where H,W are those of feauture_maps[-1]
            ...
            # if self.return_maps:
            #     maps = [out]
            # this step will append out (the feature map at level L with the
            # other feature maps at lvel L. We do an unsqueeze because out
            # lost its temporal dimension, so we bring it back.
            # Recall feature maps are of shape [b x t x encoder_widths[i] x H x W]
            # t will increase because we keep adding more predictions
            out_append = torch.unsqueeze(out, dim=1)
            feature_maps[-1] = torch.cat([feature_maps[-1], out_append], dim=1)

            for i in range(self.n_stages - 1):
                skip = self.temporal_aggregator(
                    feature_maps[-(i + 2)], pad_mask=pad_mask, attn_mask=att
                )
                # SKIP IS OUR FEATURE MAP FOR t+1
                # modify feature maps to include the new one
                skip_append = torch.unsqueeze(skip, dim=1)
                feature_maps[-(i + 2)] = torch.cat(
                    [feature_maps[-(i + 2)], skip_append], dim=1
                )
        ...
        # stack temporal predictions along first dimension
        predictions = torch.stack(predictions, dim=1)
        b, t, c, h, w = predictions.shape
        p_flat = predictions.reshape(b * t, c, h, w)
        pd_flat = self.final_downsample(p_flat)
        predictions = self.out_conv(pd_flat)
        _, k, final_h, final_w = predictions.shape
        predictions = predictions.reshape(b, t, k, final_h, final_w)
        return predictions
...
class TemporallySharedBlock(nn.Module):
    ...
    def smart_forward(self, input):
        if len(input.shape) == 4:
            ...
            if self.pad_value is not None:
                # sees what the end result shape it
        ...
        if pad_mask is not None and pad_mask.any():
            ...
            if self.mode == "att_group":
                ...
                if x.shape[-2] > w:
                    ...
                    attn = nn.AvgPool2d(kernel_size=w // x.shape[-2])(attn)
                attn = attn.view(n_heads, b, t, *x.shape[-2:])
                # x is of shape b x t x c x h x w
                # chunk splits it into n_heads x b x t x c/n_heads x h x w
                # concatenates along 0th dimension to make something of shape
                # n_heads x b x t x c/n_heads x h x w
                out = torch.stack(x.chunk(n_heads, dim=2))  # hxBxTxC/hxHxW
                # this makes it so each channel is multiplied by the attention
                # mask for the group it belongs in
                out = attn[:, :, :, None, :, :] * out
                out = out.sum(dim=2)  # sum on temporal dim -> hxBxC/hxHxW
                # out is of shape: n_heads x b x c/n_heads x h x w
                # b x c x h x w
                out = torch.cat([group for group in out], dim=1)  # -> BxCxHxW
                return out
            elif self.mode == "att_mean":
