...
class ContextConditioningStack(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        **kwargs
        ...
        self.d3 = DBlock(
            ...
            output_channels=output_channels // num_context_steps,
            conv_type=conv_type,
        )
        self.d4 = DBlock(
            input_channels=self.d3.output_channels,
            output_channels=output_channels * 2 // num_context_steps,
            conv_type=conv_type,
        )
        ...
        self.conv3 = spectral_norm(
            conv2d(
                in_channels=self.d3.output_channels * num_context_steps,
        ...
        self.convx = spectral_norm(
            conv2d(
                in_channels=input_channels * num_context_steps,
                out_channels=input_channels,
                kernel_size=3,
                padding=1,
            )
    ...
    def forward(
        ...
        # scale_1 = self._mixing_layer(scale_1, self.conv1)
        # scale_2 = self._mixing_layer(scale_2, self.conv2)
        scale_x = self._mixing_layer(x, self.convx)
        scale_3 = self._mixing_layer(scale_3, self.conv3)
        scale_4 = self._mixing_layer(scale_4, self.conv4)
        # return scale_1, scale_2, scale_3, scale_4
        return scale_x, scale_3, scale_4
