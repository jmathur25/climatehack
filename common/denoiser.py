"""
A simple denoiser that uses a pretrained neural network to fix some of the block artifacts that were found in model predictions.
"""
import os
import scipy.io
import pathlib
...
mydir = pathlib.Path(os.path.dirname(os.path.realpath(__file__)))
...
class ARCNN(nn.Module):
    def __init__(self, weight):
        ...
        self.conv1.bias.data = torch.from_numpy(
            weight["biases_conv1"].reshape(64)
        ).float()
        ...
        self.conv2.bias.data = torch.from_numpy(
            weight["biases_conv2"].reshape(32)
        ).float()
        ...
        self.conv22.bias.data = torch.from_numpy(
            weight["biases_conv22"].reshape(16)
        ).float()

        self.conv3.weight.data = torch.from_numpy(
            weight["weights_conv3"].reshape(1, 16, 5, 5).transpose(0, 1, 3, 2)
        ).float()
        self.conv3.bias.data = torch.from_numpy(
            weight["biases_conv3"].reshape(1)
        ).float()
...
class Denoiser:
    ...
    def __init__(self, device):
        weights = scipy.io.loadmat(mydir / "q20.mat")
        self.arcnn = ARCNN(weights).to(device).eval()
